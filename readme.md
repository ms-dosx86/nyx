...
Этот шаг для тех, кто не хочет покупать или не имеет собственный домен.

Зарегистрируйтесь на https://www.duckdns.org/ и создайте свой домен вида `my-domain.duckdns.org`. К этому моменту у вас должен быть сервер с публичным IP-адресом. Укажите этот адрес для только что созданного домена.

## Локальный сервер

Если у вашего интернета есть статичный IP-адрес, то вы в теории можете поднять Nyx на своем ПК. Я это не тестировал, но в теории все должно работать как и с удаленным сервером.
